        ...
        <ul className={s.header__list}  onMouseLeave={handleMouseLeave}>
          ...
          return <li key={link.name}   onMouseEnter={() => handleMouseEnter(link.name)} className="active">
            <Link 
              ...
              onClick={(e) => {
                if(link.name == t('main')) {
                  const newPathname = `/${nextLocale}${link.url}`; // Формируем новый путь с языковым префиксом и URL
                  window.location.href = newPathname; // Перезагружаем страницу с новым путем
                }
